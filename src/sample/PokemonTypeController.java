    ...
    private void damageCalculateButtonPressed(ActionEvent event) throws IOException {
        ...
        if (!keyholder2.equals("none") && (!keyholder.equals("none"))) {
            ...
            for (int i = 0; i <= result.size() - 2; i++) {
                if (result.get(i) == 1.0)
                    trueresult2.add(i, getPokeTypeList()[i] + " |" + "effective|"); //+ " " ());  //+ result.get(i).toString());
                if (result.get(i) == 0.5)
                    trueresult2.add(i, getPokeTypeList()[i] + " |" + " not very effective|"); //+ result.get(i).toString());
                //if (result.get(i) < 0.5) trueresult.add(i, typeskeyval[i]+ "dmg_resist2" + " " + result.get(i).toString());
                if (result.get(i) == 0)
                    trueresult2.add(i, getPokeTypeList()[i] + " |" + "immune|");// " " //+ result.get(i).toString());
                if (result.get(i) == 2.0) {
                    trueresult2.add(i, getPokeTypeList()[i] + " |" + "super effective|"); //+ " " + result.get(i).toString());
                    //     if (result.get(i) > 2) {trueresult.add(i, typeskeyval[i] + "dmg_weak2 " " + " " + result.get(i).toString()); }
                }
            }
            StringBuilder text = new StringBuilder();
            for (String mark : trueresult2) {
